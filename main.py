...
from tools.integrator import search_drive, list_drive_directory, read_drive_file, create_drive_folder, create_drive_file, copy_drive_item, move_drive_item, delete_drive_item, get_drive_item_info, upload_to_drive, upload_folder_to_drive, download_from_drive, share_drive_file, read_unread_emails, get_mailbox_status, create_quick_draft
from tools.analyst import get_channel_history
...
async def main():
    ...
    bary_core.register_dynamic_agent(
        ...
        agent_purpose="You are Google Cloud Bot (DGDB). Your task is to interact with Google Drive API to search, read, "
        "and manipulate cloud files.",
        tools=[search_drive, list_drive_directory, read_drive_file, create_drive_folder, create_drive_file, copy_drive_item, 
    ...
    bary_core.register_static_agent(
        agent_id="@SGRB",
        commands_map={
            "непрочитані": read_unread_emails,
